...
describe("Motivational Speech API 'POST' request", () => {
  ...
  it("should add 1 to the motivational speech resource count", () => {
    let motivationalSpeechesResourceCount;

    cy.request("http://localhost:8000/api/motivational-speeches/").then(
      (response) => {
        motivationalSpeechesResourceCount = response.body.length;
      }
    );
    ...
    cy.request("http://localhost:8000/api/motivational-speeches/").then(
      (response) => {
        expect(response.body.length).to.eq(
          motivationalSpeechesResourceCount + 1
        );
      }
    );
  });
